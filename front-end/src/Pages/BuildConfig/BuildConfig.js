...
import {getProductByTypeProductId, getAccessories} from "../../apis/buildConfig";
...
function BuildConfig() {
  ...
  let priceCPU = config.cpu === undefined ? 0 : qtyCPU * config.cpu.GiaKM;
  let priceRAM = config.ram === undefined ? 0 : qtyRAM * config.ram.GiaKM;
  let priceMain = config.main === undefined ? 0 : qtyMain * config.main.GiaKM;
  let priceMonitor = config.monitor === undefined ? 0 : qtyMonitor * config.monitor.GiaKM;
  let priceStorage = config.storage === undefined ? 0 : qtyStorage * config.storage.GiaKM;
  let pricePower = config.power === undefined ? 0 : qtyPower * config.power.GiaKM;
  let priceVGA = config.vga === undefined ? 0 : qtyVGA * config.vga.GiaKM;
  let priceCooler = config.cooler === undefined ? 0 : qtyCooler * config.cooler.GiaKM;
  ...
  const handleShowAccessories = (data) => {
    setIdConfig(data.id);
    getProductByTypeProductId({id: data.id, page: page})
      .then((response) => {
...
useEffect(() => {
  getProductByTypeProductId({id: idConfig, page: page})
    .then((response) => {
      setAccessories(response.data);
      setTotalPage(response.last_page);
    });
  ...
  useEffect(() => {
    getAccessories("typeCPU").then((response) => {
      setTypeCPU(response);
      setLoading(false);
    });

    getAccessories("typeRAM").then((response) => {
      setTypeRAM(response);
      setLoading(false);
    });
    getAccessories("typeMainBoard").then((response) => {
      setTypeMain(response);
      setLoading(false);
    });
    getAccessories("typeMonitor").then((response) => {
      setTypeMonitor(response);
      setLoading(false);
    });
    getAccessories("typeStorage").then((response) => {
      setTypeStorage(response);
      setLoading(false);
    });
    getAccessories("typePower").then((response) => {
      setTypePower(response);
      setLoading(false);
    });
    getAccessories("typeVGA").then((response) => {
      setTypeVGA(response);
      setLoading(false);
    });
    getAccessories("typeCooler").then((response) => {
      setTypeCooler(response);
      setLoading(false);
    });
  }, []);
