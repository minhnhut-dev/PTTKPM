...
import {GetProductByID, GetImageProductByID, getComments, suggestProducts, AddComment} from "../../apis/products";
...
useEffect(() => {
  suggestProducts(Product?.loai_san_phams_id).then((response) => {
    setSuggestProduct(response);
  });
},[Product]);
  ...
  const handlePostComment = () => {
    ...
    if (content === '')
    {
      enqueueSnackbar("vui lòng nhập nội dung bình luận", {
        variant: "error",
      ...
      .then((response) => {
        getComments(id)
          .then((response) => {
            enqueueSnackbar("Bình luận thành công ", {
              variant: "success",
              autoHideDuration: 3000,
              preventDuplicate: true,
            });
  ...
  // Product.forEach((element) => {
    if (Product?.CauHinh === "" || Product?.CauHinh === undefined) {
      elements.push(
              ...
              <div className="page_content">
                <div className="row">
                  <div className="col-sm-6 col-xs-12 product_thumbnail">
                    ...
                    >
                      <h1 className="product_name">{Product?.TenSanPham}</h1>
                      ...
                      <p className="Brand-product">
                        <span>Hãng sản xuất : {Product?.HangSanXuat}</span>
                      </p>
                  ...
                  <Tabs>
                    ...
                    <TabPanel>
                      ...
                      {/*phần đánh giá chi tiết */}
                        <div
                          dangerouslySetInnerHTML={{ __html: Product?.ThongTin }}
